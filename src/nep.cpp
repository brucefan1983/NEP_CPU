...
void find_dftd3_coordination_number_for_lammps(
  NEP3::DFTD3& dftd3, int N, int* g_ilist, int* g_NN, int** g_NL, int* g_type, double** g_pos)
{
  for (int ii = 0; ii < N; ++ii) {
    int n1 = g_ilist[ii];
    int type1 = g_type[n1] - 1; // from LAMMPS to NEP convention
    int z1 = dftd3.atomic_number[type1];
    double R_cov_1 = dftd3para::Bohr * dftd3para::covalent_radius[z1];
    double cn_temp = 0.0;
    for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
      int n2 = g_NL[n1][i1];
      double r12[3] = {
        g_pos[n2][0] - g_pos[n1][0], g_pos[n2][1] - g_pos[n1][1], g_pos[n2][2] - g_pos[n1][2]};
      double d12sq = r12[0] * r12[0] + r12[1] * r12[1] + r12[2] * r12[2];
      if (d12sq >= dftd3.rc_angular * dftd3.rc_angular) {
        continue;
      }
      int type2 = g_type[n2] - 1; // from LAMMPS to NEP convention
      int z2 = dftd3.atomic_number[type2];
      double R_cov_2 = dftd3para::Bohr * dftd3para::covalent_radius[z2];
      double d12 = sqrt(d12sq);
      cn_temp += 1.0 / (exp(-16.0 * ((R_cov_1 + R_cov_2) / d12 - 1.0)) + 1.0);
    }
...
void add_dftd3_force_for_lammps(
  NEP3::DFTD3& dftd3,
  int N,
  int* g_ilist,
  int* g_NN,
  ...
  for (int ii = 0; ii < N; ++ii) {
    ...
    for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
      ...
      double r12[3] = {
        g_pos[n2][0] - g_pos[n1][0], g_pos[n2][1] - g_pos[n1][1], g_pos[n2][2] - g_pos[n1][2]};
      double d12_2 = r12[0] * r12[0] + r12[1] * r12[1] + r12[2] * r12[2];
      if (d12_2 >= dftd3.rc_radial * dftd3.rc_radial) {
        continue;
      }
      int type2 = g_type[n2] - 1; // from LAMMPS to NEP convention
      int z2 = dftd3.atomic_number[type2];
      ...
      double damp_6 = 1.0 / (d12_6 + damp_4 * damp_2);
      double damp_8 = 1.0 / (d12_8 + damp_4 * damp_4);
      double f = -(dftd3.s6 * c6 * damp_6 + dftd3.s8 * c8 * damp_8);
      double f2 = dftd3.s6 * c6 * 3.0 * d12_4 * (damp_6 * damp_6) +
                  dftd3.s8 * c8 * 4.0 * d12_6 * (damp_8 * damp_8);
      double f12[3] = {r12[0] * f2, r12[1] * f2, r12[2] * f2};
...
void add_dftd3_force_extra_for_lammps(
  NEP3::DFTD3& dftd3,
  int N,
  int* g_ilist,
  int* g_NN,
  ...
  for (int ii = 0; ii < N; ++ii) {
    ...
    for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
      ...
      double r12[3] = {
        g_pos[n2][0] - g_pos[n1][0], g_pos[n2][1] - g_pos[n1][1], g_pos[n2][2] - g_pos[n1][2]};
      double d12_2 = r12[0] * r12[0] + r12[1] * r12[1] + r12[2] * r12[2];
      if (d12_2 >= dftd3.rc_angular * dftd3.rc_angular) {
        continue;
      }
      int type2 = g_type[n2] - 1; // from LAMMPS to NEP convention
      int z2 = dftd3.atomic_number[type2];
...
void NEP3::init_d3(std::string& xc_functional, double rc_d3, double rc_d3_cn, const bool is_rank_0)
{
  dftd3.enabled = true;
  ...
  dftd3.rc_radial = rc_d3;
  dftd3.rc_angular = rc_d3_cn;
  if (is_rank_0) {
    std::cout << "    add DFT-D3" << std::endl;
    std::cout << "        xc_functional = " << xc_functional << std::endl;
    std::cout << "        potential cutoff = " << rc_d3 << " A" << std::endl;
    std::cout << "        coordination number cutoff = " << rc_d3_cn << " A" << std::endl;
  }
}
  ...
  double** virial)
{
  ...
  if (dftd3.enabled) {
    find_dftd3_coordination_number_for_lammps(dftd3, N, ilist, NN, NL, type, pos);
    add_dftd3_force_for_lammps(
      dftd3, N, ilist, NN, NL, type, pos, force, total_virial, virial, total_potential, potential);
    add_dftd3_force_extra_for_lammps(
      dftd3, N, ilist, NN, NL, type, pos, force, total_virial, virial);
  }
}
